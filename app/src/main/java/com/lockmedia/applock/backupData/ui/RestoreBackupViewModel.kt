...
import com.lockmedia.applock.other.extensions.remove
import com.lockmedia.applock.other.getFileName
import com.lockmedia.applock.other.getFileSize
...
class RestoreBackupViewModel @Inject constructor(
    ...
    var restoreState: RestoreState = RestoreState.INITIALIZE
        set(value) {
            field = value
            notifyChange(com.lockmedia.applock.BR.restoreState, value)
        }
    ...
    var metaData: BackupMetaData? = null
        set(value) {
            field = value
            notifyChange(com.lockmedia.applock.BR.metaData, value)
        }
    ...
    var zipFileName: String = String.empty
        set(value) {
            field = value
            notifyChange(com.lockmedia.applock.BR.zipFileName, value)
        }
    ...
    var backupSize: Long = 0
        set(value) {
            field = value
            notifyChange(com.lockmedia.applock.BR.backupSize, value)
        }
    ...
    fun loadAndValidateBackup(uri: Uri) = viewModelScope.launch(Dispatchers.IO) {
        ...
        createStream(fileUri)?.use { stream ->
            ...
            while (ze != null) {
                ...
                } else if (ze.name.endsWith(".applock")) {
                    photoFiles++
                }
    ...
    private suspend fun restoreVersion2(stream: ZipInputStream, origPassword: String) {
        ...
        while (ze != null) {
            ...
            val oldUUID = ze.name.remove(".applock").remove(".tn").remove(".vp")
